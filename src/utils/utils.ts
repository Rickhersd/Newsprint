...
export function invertGameBoard(gameBoard: GameBoard<GameBoardCell<number>>): GameBoard<GameBoardCell<number>> {
...
export function checkAllCorrect(gameBoard: GameBoard<GameBoardCell<number>>): boolean {
  for(let rowIndex = 0; rowIndex < gameBoard.length; rowIndex++){
    for(let cellIndex = 0; cellIndex < gameBoard.length; cellIndex++){
      if (gameBoard[rowIndex][cellIndex].state !== 'correct') return false;
    }
  }
